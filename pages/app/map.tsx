...
import { 
  ...
  FiCloud, FiFilter, FiChevronDown
} from 'react-icons/fi';
import { 
  IoRestaurant, IoCafe, IoBeer, IoCarSport, IoStorefront,
  IoThumbsUp, IoTrendingUp, IoWarning, IoClose
              ...
              places.map((place) => (
                <div key={place.id} className="place-card" onClick={() => router.push(`/app/place/${place.slug || place.id}`)}>
                  <div className="place-image">
                    <img 
                      src={place.photo_url || 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=400&h=300&fit=crop'} 
                      alt={place.name}
                    />
                  </div>
                  <div className="place-info">
                    <h3 className="place-name">{place.name}</h3>
                    <p className="place-category">{place.category} • {place.city || 'Nearby'}</p>
                  </div>
