...
import {canUseTemplate,readTemplateSelection,templateCardType,templateSelectionPath,browseCategoryForTemplate,browseCategoryForCardType,firstBrowseSelection,BrowsePlan} from '../../../lib/ecard/templateSelection';
...
export default function ECardNewPage(){
  ...
  const [galleryFilter,setGalleryFilter]=useState('business-services'),[planFilter,setPlanFilter]=useState<BrowsePlan>('all');
  ...
  useEffect(()=>{
    ...
    if(preset){setSelectedTemplateId(preset.template.id);setSelectedColorSchemeId(preset.scheme.id);setCardType(preset.cardType);setGalleryFilter(browseCategoryForTemplate(preset.template.id));setSetup(value=>({...value,primaryColor:preset.scheme.primary}));}
  ...
  const selectDesign=(id:string,color:string)=>{const selection=readTemplateSelection(id,color);if(!selection)return;setSelectedTemplateId(id);setSelectedColorSchemeId(selection.scheme.id);if(selectedTemplateId!==id){setCardType(selection.cardType);if(selection.cardType!=='politician'){setCountryCode(undefined);setCountryTemplate(undefined);}}setSetup(value=>({...value,primaryColor:selection.scheme.primary}));};
  const selectBrowse=(category:string,nextPlan:BrowsePlan)=>{if(category===galleryFilter&&nextPlan===planFilter)return;setGalleryFilter(category);setPlanFilter(nextPlan);setTemplateAvailable(false);const chosen=firstBrowseSelection(category,plan.isPro,selectedTemplateId||undefined,nextPlan);if(chosen&&canUseTemplate(chosen.template,chosen.scheme.id,plan.isPro)){const color=nextPlan!=='free'&&chosen.template.id===selectedTemplateId?chosen.template.colorSchemes.find(item=>item.id===selectedColorSchemeId)||chosen.scheme:chosen.scheme;selectDesign(chosen.template.id,color.id);}else{setSelectedTemplateId(null);setSelectedColorSchemeId(null);}};
  const selectType=(type:string,country?:string,countryTpl?:string)=>{if(!['business','personal','politician'].includes(type))return;const category=browseCategoryForCardType(type);setCardType(type);setGalleryFilter(category);setPlanFilter('all');setCountryCode(country);setCountryTemplate(countryTpl);setTemplateAvailable(false);const chosen=firstBrowseSelection(category,plan.isPro,countryTpl);const allowed=chosen&&canUseTemplate(chosen.template,chosen.scheme.id,plan.isPro);setSelectedTemplateId(allowed?chosen.template.id:null);setSelectedColorSchemeId(allowed?chosen.scheme.id:null);if(allowed)setSetup(value=>({...value,primaryColor:chosen.scheme.primary}));go('template');};
  const quickStart=()=>{const basic=getTemplateById('basic')!;const color=basic.colorSchemes.find(item=>item.isFree)||basic.colorSchemes[0];setSelectedTemplateId(basic.id);setSelectedColorSchemeId(color.id);setCountryCode(undefined);setCountryTemplate(undefined);setCardType('business');setGalleryFilter('personal-creators');setPlanFilter('all');setSetup(value=>({...value,primaryColor:color.primary}));go('setup');};
  ...
  return <><Head><title>{copy('Create eCard')} | Tavvy</title></Head><AppLayout hideTabBar><main className={`ecard-create ${isDark?'dark':''}`}>
    ...
    <div className="setup-content">
      {loading||!router.isReady||!presetReady?<p role="status">{copy('Loading your account…')}</p>:!user?<section><h1>{copy('Create your eCard')}</h1><p>{copy('Sign in to save your card as a private draft.')}</p><button className="primary" onClick={()=>void router.push('/app/login?redirect='+encodeURIComponent(selectedTemplateId&&selectedColorSchemeId?templateSelectionPath(selectedTemplateId,selectedColorSchemeId):'/app/ecard/new'))}>{copy('Sign in')}</button></section>:<>
        ...
        {step==='template'&&<><div className="creation-paths"><button type="button" onClick={quickStart}>{copy('Quick setup')}</button><button type="button" onClick={()=>go('type')}>{copy('Choose card type & template')}</button></div><TemplateBrowseFilters category={galleryFilter} plan={planFilter} onChange={selectBrowse}/><TemplateGallery key={`${galleryFilter}:${planFilter}:${countryTemplate||''}`} cardType={galleryFilter} planFilter={planFilter} countryTemplate={countryTemplate} selectedTemplateId={selectedTemplateId} selectedColorSchemeId={selectedColorSchemeId} onSelect={selectDesign} onBack={()=>go('type')} onAvailabilityChange={setTemplateAvailable} isPro={plan.isPro} isDark={isDark}/><button className="primary continue" disabled={!selectedTemplateId||!templateAvailable||plan.loading||!!plan.error} onClick={()=>go('setup')}>{copy('Use this template')}</button></>}
      </>}
    </div>
