...
export default function RealtorDetailScreen() {
  ...
  if (loading) return <AppLayout><div style={{minHeight:'100vh',background:'#0A0A0F',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{width:'32px',height:'32px',border:'3px solid #252532',borderTopColor:'#8A05BE',borderRadius:'50%',animation:'spin 1s linear infinite'}} /></div></AppLayout>;
        ...
        <div style={{minHeight:'100vh',background:'#0A0A0F',paddingBottom:'100px'}}>
          ...
          <div style={{padding:'20px'}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px'}}>
              ...
              {realtor.is_verified && <div style={{background:'#8A05BE',borderRadius:'50%',padding:'4px'}}><FiCheck size={14} color="#FFFFFF" /></div>}
            </div>
            ...
            <div style={{display:'flex',gap:'12px',marginBottom:'24px'}}>
              <button style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',background:'linear-gradient(90deg, #8A05BE 0%, #6B0496 100%)',color:'#FFFFFF',padding:'14px',borderRadius:'12px',border:'none',fontSize:'16px',fontWeight:600,cursor:'pointer'}}><FiPhone size={18} /> Call</button>
            ...
            <div style={{display:'flex',borderBottom:'1px solid #252532',marginBottom:'20px'}}>
              {['about', 'listings', 'reviews'].map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} style={{flex:1,padding:'14px',background:'transparent',border:'none',borderBottom:activeTab === tab ? '2px solid #8A05BE' : '2px solid transparent',color:activeTab === tab ? '#FFFFFF' : '#6B7280',fontSize:'14px',fontWeight:600,cursor:'pointer',textTransform:'capitalize'}}>{tab}</button>))}
            </div>
