...
The homepage now exposes a Where field. When a user enters a city, its `where` route parameter takes precedence over current GPS coordinates; an empty Where field retains the existing nearby behavior. TypeScript, production build and phone-width screenshot passed. Commit `f8d3cce` deployed as Railway `40aa2004-f0f0-4065-99d7-0e59f93ca676` SUCCESS; a live phone-width browser check sent Boston, MA without GPS route parameters. The matching native home already has an explicit location field; its visual icon/carousel update is being checked separately.
