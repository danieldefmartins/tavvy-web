...
const base = process.env.TAVVY_PREVIEW_BASE || 'http://127.0.0.1:3842';
const out = '/private/tmp/tavvy-place-preview-20260921';
...
(async () => {
  ...
  try {
    for (const theme of ['light','dark']) {
      ...
      const first = await page.$eval('article.card', e => ({text:e.innerText,src:e.querySelector('img').getAttribute('src'),tiles:e.querySelectorAll('[data-review-tile]').length,overflow:e.scrollWidth>e.clientWidth}));
      assert.match(first.text,/9.9 mi/); assert.doesNotMatch(first.text,/16000/);
      assert.match(first.text,/69 Example Street, Boston, MA/);
      assert.match(first.text,/12 recent reviewers/); assert.equal(first.tiles,4); assert.equal(first.overflow,false);
    ...
    assert.equal(await page.$eval('article.card .photos',e=>e.scrollWidth>e.clientWidth),true);
    unavailable=true;
    await page.reload({waitUntil:'networkidle2'});
    await page.waitForSelector('article.card');
    assert.equal(await page.$eval('article.card',e=>(e.innerText.match(/Recent reviews unavailable/g)||[]).length),1);
    assert.equal(await page.$('article.card [data-review-tile]'),null);
    ...
    await page.waitForSelector('article.card');
    assert.equal(await page.$eval('article.card .photos',e=>e.children.length),3);
    ...
    console.log('PASS: light/dark cards, four review sections, real counts, address, meters conversion, zero distance, no overflow, real-photo replacement');
