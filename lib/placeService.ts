import { searchDatabase } from './placeSearch';
import { resolveSearchIntent, SearchContext } from './searchIntent';
/**
 * placeService.ts
 * 
 ...
 */
export async function searchPlaces(query: string, userLocation?: [number, number] | null, limit = 20): Promise<PlaceCard[]> {
  return searchSuggestions(query, limit, userLocation ? { latitude: userLocation[1], longitude: userLocation[0] } : undefined);
}

export async function searchSuggestions(query: string, limit = 8, userLocation?: { latitude: number; longitude: number }, includeFsq = true, context: SearchContext = {}): Promise<SearchResult[]> {
  if (!query.trim()) return [];
  const intent = resolveSearchIntent(query, { coordinates: userLocation, ...context });
  return await searchDatabase(intent, limit, includeFsq) as SearchResult[];
}
