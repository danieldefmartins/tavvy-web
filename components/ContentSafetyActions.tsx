...
export default function ContentSafetyActions({ kind, contentId, onChanged }: { kind: ContentKind; contentId: string; onChanged?: () => void }) {
 ...
 return <div className="safety" onClick={event=>event.stopPropagation()}>
  <button type="button" aria-expanded={mode!=='closed'} onClick={()=>setMode(mode==='closed'?'menu':'closed')} disabled={busy}>{copy('Report or block')}</button>
