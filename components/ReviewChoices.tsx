      ...
      return <section className={section.key} key={section.key} aria-label={copy(section.title)}>
        ...
        {open && <div id={`${id}-${section.key}`} className="section-body">
          <div className="choices">{visible.map(choice => {
            ...
            return <button type="button" className={`choice ${intensity ? 'selected' : ''}`} key={choice.id} aria-pressed={!!intensity} disabled={disabled || (atLimit && !intensity)} onClick={() => onChange(toggleComposerChoice(selected, choice))}>
