...
import { cleanAddressLabel, withTypedHouseNumber } from '../../lib/addPlaceFlow';
  ...
  const searchAddresses = useCallback(async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&addressdetails=1&limit=8&countrycodes=us&layer=address,poi`
      );
      const data: NominatimResult[] = await response.json();
      // Addresses only: clean labels, no business names, no duplicates.
      const seen = new Set<string>();
      setSuggestions((Array.isArray(data) ? data : []).map(raw => withTypedHouseNumber(raw as any, query)).map(item => ({ ...item, display_name: cleanAddressLabel((item as any).address) || '' })).filter(item => { if (!item.display_name || seen.has(item.display_name)) return false; seen.add(item.display_name); return true; }));
