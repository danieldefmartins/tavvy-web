...
export default function ProviderReviewForm({providerId,kind,signedIn,onSaved}:{providerId:string;kind:ProviderTapKind;signedIn:boolean;onSaved:()=>void}){
  ...
  return <section aria-labelledby="write-provider-review"><h2 id="write-provider-review">Review this {kind==='realtor'?'Realtor':'professional'}</h2>{!signedIn?<p><Link href="/app/login">Sign in</Link> to share your experience.</p>:<form onSubmit={submit}>
    <p>Choose what mattered to you. You can skip any part that did not apply.</p>
    ...
    <label style={{display:'block',margin:'12px 0'}}>Overall experience <select required value={rating} onChange={e=>setRating(Number(e.target.value))}><option value={0}>Choose a rating</option>{[5,4,3,2,1].map(n=><option value={n} key={n}>{n} / 5</option>)}</select></label>
    ...
    <label style={{display:'block',margin:'12px 0'}}>What happened? <textarea required minLength={20} maxLength={4000} value={content} onChange={e=>setContent(e.target.value)} rows={5} style={{display:'block',width:'100%'}}/></label>
  ...
  <style jsx>{`fieldset button{border:1px solid #888;border-radius:999px;background:transparent;color:inherit;padding:9px 13px;min-height:42px;cursor:pointer}fieldset button[aria-pressed="true"]{border-color:#009888;background:#e1f6ef;color:#123b36;font-weight:700}select,input,textarea{font:inherit;padding:10px;border:1px solid #888;border-radius:8px}form>button{min-height:44px;padding:10px 18px;border:0;border-radius:10px;background:#008d7a;color:white;font:inherit;font-weight:700}`}</style>
  </section>;
}
