...
export default function DemoReview({ onClose, onSave }: { onClose: () => void; onSave: (review: EvidenceVisit) => void }) {
  ...
  return <DemoDialog title="What was your experience?" onClose={onClose}>
    <p>Try Tavvy’s taps. Your choices update this demo’s review summary on this device.</p>
    ...
    <button className="demo-primary" disabled={!selected.length} onClick={() => onSave({ reviewId: 'demo-your-review', userId: 'demo-you', visitedAt: new Date().toISOString(), signals: demoReviewChoices.flatMap(g => [...g.choices]).filter(c => selected.includes(c.slug)) })}>Add {selected.length || ''} tap{selected.length === 1 ? '' : 's'} to the demo</button>
