...
export default function DemoContact() {
  ...
  return <section className="contact-demo">
    ...
    {action && <DemoDialog title={action} onClose={() => setAction(null)}>
      {action === 'Reserve a table' ? reserved ? <><p><strong>Your reservation is confirmed.</strong></p><p>{date} · A table for your evening.</p><button className="demo-primary" onClick={() => setAction(null)}>Back to the restaurant</button></> : <form onSubmit={e => { e.preventDefault(); setReserved(true); }}>
        <p>Choose your evening and we will hold a table.</p>
        ...
        <label>Party size<select>{[2,3,4,5,6].map(n => <option key={n}>{n} guests</option>)}</select></label><button className="demo-primary">Reserve</button>
      </form> : action === 'Call' ? <><p>Call us at <strong>(407) 555-0142</strong>.</p><a className="demo-primary" href="tel:+14075550142">Call (407) 555-0142</a></> : ['DoorDash','Uber Eats','Grubhub'].includes(action) ? <><p>{action} delivery is not available right now.</p><a className="demo-primary" href={DEMO_ORDER}>Order at your table</a></> : <><p>Trattoria Tavvy on {action}: the food, the room and the stories from our kitchen.</p><a className="demo-primary" href={`${DEMO_HOME}?tab=media`}>See photos & stories</a></>}
    </DemoDialog>}
