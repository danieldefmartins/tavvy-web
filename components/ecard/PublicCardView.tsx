import type { CardLink, FormField, FormBlockData, CardData, PageProps } from '../../lib/ecard/publicCardTypes';
/**
...
import CivicCardSection from '../CivicCardSection';
import ECardMessageButton from '../ECardMessageButton';
import { useTranslation } from 'next-i18next';
import ContentSafetyActions from '../ContentSafetyActions';
import {usePublicCardSafety} from '../../hooks/usePublicCardSafety';

/** Preserve chosen colors when legible; use a readable glyph on solid surfaces. */
function readableOnSolid(foreground: string, background: string, minimum = 3): string {
  const luminance = (input: string): number | null => {
    const hex = /^#[0-9a-f]{3}$/i.test(input) ? `#${input.slice(1).split('').map(c => c + c).join('')}` : input;
  ...
  return contrast(luminance('#111827')!, bg) >= contrast(1, bg) ? '#111827' : '#ffffff';
}

/** Resolve a translucent decorator against its actual card surface. */
function solidSurface(color: string, backdrop: string): string {
  if (/^#[0-9a-f]{6}$/i.test(color)) return color;
  if (color === 'transparent') return backdrop;
  const hex = /^#([0-9a-f]{6})([0-9a-f]{2})$/i.exec(color);
  const rgba = /^rgba\(\s*(\d+),\s*(\d+),\s*(\d+),\s*([\d.]+)\s*\)$/i.exec(color);
  if ((!hex && !rgba) || !/^#[0-9a-f]{6}$/i.test(backdrop)) return color;
  const tint = hex ? [0, 2, 4].map(i => parseInt(hex[1].slice(i, i + 2), 16)) : rgba!.slice(1, 4).map(Number);
  const alpha = hex ? parseInt(hex[2], 16) / 255 : Math.min(1, Math.max(0, Number(rgba![4])));
  return '#' + tint.map((channel, i) => Math.round(channel * alpha + parseInt(backdrop.slice(1 + i * 2, 3 + i * 2), 16) * (1 - alpha)).toString(16).padStart(2, '0')).join('');
}
...
export default function PublicCardView({ cardData: initialCardData, error: initialError, isPreview = false, previewOnly = false }: PageProps) {
  const { t } = useTranslation();
  const [storedCardData, setCardData] = useState<CardData | null>(initialCardData);
  const safety = usePublicCardSafety(initialCardData?.id, previewOnly, (initialCardData?.recentEndorsements || []).filter(note=>!!note.id) as any);
  const baseCardData = previewOnly ? initialCardData : storedCardData;
  const cardData = !previewOnly && baseCardData ? {...baseCardData,recentEndorsements:safety.endorsements} : baseCardData;
  ...
  const [endorsementTotalsUnavailable,setEndorsementTotalsUnavailable]=useState(false);

  // Studio previews never submit a pending public endorsement.
  useEffect(() => {
      ...
      const submitPending = async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            // Use confirmed public totals only; the write can succeed while a read fails.
            setEndorsementTotalsUnavailable(resData.evidenceStatus==='unavailable'||!Number.isInteger(resData.endorsementCount));
            setCardData(prev => prev ? {
              ...prev,
              endorsementCount: Number.isInteger(resData.endorsementCount) ? resData.endorsementCount : prev.endorsementCount,
              tapCount: prev.tapCount,
  ...
  if (!previewOnly && initialCardData && safety.status !== 'ready') {
    return <main style={{padding:24,maxWidth:480,margin:'40px auto',fontFamily:'system-ui'}}><Head><title>{initialCardData.fullName} · Tavvy</title></Head><p role={safety.status==='unavailable'?'alert':'status'}>{safety.status==='loading'?'Loading card…':safety.status==='hidden'?'This card is not available in your view.':'This card could not be loaded. Please try again.'}</p>{safety.status==='unavailable'&&<button onClick={safety.reload} style={{minHeight:44}}>Try again</button>}<a href="/app/settings">Content settings</a></main>;
  }
  if (error || !cardData) {
    return (
  ...
  const isCivicCard = templateLayout.startsWith('civic-card') || templateLayout === 'politician-generic';
  const isNonClassicCivic = templateLayout.startsWith('civic-card') && templateLayout !== 'civic-card';
  ...
  // A selected canvas image is a user choice, not a template default. Keep
  // readable card panels while honoring that choice behind every layout.
  const customBackgroundImage = cardData.backgroundType === 'image' ? cardData.backgroundImageUrl : null;
  const heroImage = cardData.bannerImageUrl || cardData.profilePhotoUrl || customBackgroundImage;
  const flagBackgroundImage = customBackgroundImage || cardData.bannerImageUrl;
  ...
  const proBodyIsLight = isLightBg(activeColorScheme?.cardBg || '#ffffff');
  const proBodyText = proBodyIsLight ? '#334155' : '#e2e8f0';
  const proBodyMuted = proBodyIsLight ? '#64748b' : '#b6c2d2';
  const proBodyBorder = proBodyIsLight ? '#e8ecf0' : 'rgba(255,255,255,0.14)';
  const proBodyAccent = readableOnSolid(templateStyles.accentColor, activeColorScheme?.cardBg || '#ffffff', 3);
  const secondaryText = bgIsActuallyLight ? '#526071' : '#d1d9e5';
  // Translucent buttons share the card surface. Resolve their tint against that
  // surface so pale accents remain visible on light cards as well.
  const canvasColor = bgString.match(/#[0-9a-f]{6}/i)?.[0] || cardData.gradientColor1 || '#111827';
  const hasWhiteSocialPanel = ['business-card', 'cover-card', 'biz-traditional', 'biz-modern', 'biz-minimalist', 'mobile-business'].includes(templateLayout) || (isCivicCard && !isFlagCivic);
  const socialSurfaceColor = hasWhiteSocialPanel ? '#ffffff'
    : templateLayout === 'pro-card' || templateLayout === 'blogger'
      ? solidSurface(activeColorScheme?.cardBg || '#ffffff', canvasColor)
      : templateLayout === 'premium-static' ? cardData.gradientColor2 || canvasColor : canvasColor;
  const socialButtonBackground = solidSurface(templateStyles.buttonBg, socialSurfaceColor);
  const socialButtonIconColor = readableOnSolid(socialIconColor, socialButtonBackground);
  ...
  const isLightFooterBg = templateLayout === 'pro-card' ? proBodyIsLight : (templateLayout === 'cover-card' || templateLayout === 'biz-traditional' || templateLayout === 'biz-modern' || templateLayout === 'biz-minimalist' || templateLayout === 'mobile-business' || isCivicCard) ? true : bgIsActuallyLight;
  ...
  return (
    <>
      ...
      <div style={{...styles.page, ...(templateLayout === 'premium-static' ? { flexDirection: 'column' as const, alignItems: 'center' } : {}), ...(previewOnly ? {position: 'relative' as const, minHeight: 0} : {})}}>
        {/* Background - supports solid, gradient, image, and template layouts */}
        <div 
          style={{
            ...styles.backgroundGradient,
            ...(customBackgroundImage
              ? { backgroundImage: `url(${JSON.stringify(customBackgroundImage)})`, backgroundSize: 'cover', backgroundPosition: 'center' }
              : (templateLayout === 'full-width') && heroImage
              ? {
              ...
              : templateLayout === 'premium-static'
                ? { background: cardData.gradientColor2 || activeColorScheme?.secondary || '#101827' }
            ...
            {heroImage ? (
              ...
              <div style={{
                ...
                <div style={{ textAlign: 'center' as const, padding: '40px' }}>
                  ...
                  {/* Initials are a finished public fallback; editing guidance belongs in the editor. */}
                </div>
              </div>
            )}
        ...
        <div style={{
          ...
          ...(templateLayout === 'biz-minimalist' ? {
            maxWidth: '420px',
            margin: '20px auto',
            padding: cardData.bannerImageUrl ? '28px 28px 24px' : '72px 28px 24px',
            borderRadius: '16px',
            overflow: 'hidden' as const,
          ...
          {isFlagCivic && (
            <>
              <div style={{
                ...
                ...(flagBackgroundImage
                  ? {
                      backgroundImage: `url(${JSON.stringify(flagBackgroundImage)})`,
                      backgroundSize: 'cover',
                ...
                opacity: flagBackgroundImage ? 0.85 : 1,
                zIndex: 0,
              }} />
              ...
              <div style={{
                ...
                right: 0,
                bottom: 0,
                background: flagBackgroundImage
                  ? 'linear-gradient(180deg, rgba(0,20,10,0.60) 0%, rgba(0,20,10,0.58) 40%, rgba(0,20,10,0.65) 100%)'
          ...
          {(() => {
            ...
            return (
              <button 
                ...
                style={{
                  ...styles.crownButton,
                  // Premium Hero overlaps its content onto the photo by80px.
                  // Keep engagement in the photo corner, clear of the name.
                  ...(templateLayout === 'premium-static' ? { top: `calc(100px - ${heroImage ? '60vh' : '40vh'})` } : {}),
              ...
              >
                ...
                <span style={{
                  ...
                  fontSize: '17px',
                  fontWeight: '700',
                }}>{endorsementTotalsUnavailable ? '—' : cardData.endorsementCount ?? cardData.tapCount ?? 0}</span>
          ...
          {(templateLayout === 'pro-realtor' || templateLayout === 'pro-card' || templateLayout === 'pro-creative') && cardData.bannerImageUrl && (
            <div style={{
              ...
              borderRadius: '20px',
              overflow: 'hidden',
              marginBottom: templateLayout === 'pro-realtor' ? '-50px' : templateLayout === 'pro-card' ? '0' : '16px',
              position: 'relative',
              zIndex: 2,
            }}>
              <img
            ...
            <div style={{
              ...
              {cardData.company && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' as const, color: readableOnSolid(templateStyles.accentColor, activeColorScheme?.primary || cardData.gradientColor1, 4.5) }}>{cardData.company}</span>
                </div>
              )}
            </div>
          )}
          ...
          {templateLayout === 'pro-card' && (
            <div style={{
              background: activeColorScheme?.background || `linear-gradient(165deg, ${cardData.gradientColor1} 0%, ${cardData.gradientColor2} 100%)`,
              padding: cardData.bannerImageUrl ? '24px 28px 40px' : '76px 28px 40px',
              width: '100%',
              position: 'relative' as const,
              minHeight: cardData.profilePhotoUrl ? 232 : undefined,
            }}>
              ...
              {cardData.company && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
                  ...
                  <div>
                    ...
                    {cardData.title && <div style={{ fontSize: 11, color: templateStyles.accentColor, letterSpacing: 1, textTransform: 'uppercase' as const, fontWeight: 600 }}>{cardData.professionalCategory || cardData.title}</div>}
                  </div>
                </div>
              )}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                <div style={{ flex: 1, paddingRight: 8 }}>
                  <h1 style={{ fontSize: 26, fontWeight: 700, color: cardData.fontColor || '#ffffff', margin: '0 0 4px 0', lineHeight: 1.2 }}>{cardData.fullName}</h1>
                  ...
                  {cardData.title && <p style={{ fontSize: 14, color: '#e2e8f0', fontWeight: 500, margin: '8px 0 0 0' }}>{cardData.title}</p>}
          ...
          {templateLayout === 'cover-card' && (
            <div style={{ position: 'relative', width: '100%' }}>
              {/* Cover photo */}
              <div style={{
                width: '100%', height: heroImage ? 280 : 96, position: 'relative',
          ...
          {templateLayout === 'biz-traditional' && (
            <div style={{ width: '100%' }}>
              ...
              {/* Logo area */}
              <div style={{ padding: cardData.bannerImageUrl ? '20px 28px 0' : '64px 28px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
                {cardData.companyLogoUrl ? (
          ...
          {templateLayout === 'biz-modern' && (
            <div style={{ width: '100%' }}>
              <div style={{
                background: cardData.bannerImageUrl ? 'none' : `linear-gradient(135deg, ${activeColorScheme?.primary || '#0f2b5b'} 0%, ${activeColorScheme?.secondary || '#1a3f7a'} 100%)`,
                padding: '0',
                position: 'relative' as const,
                overflow: 'visible',
              }}>
                ...
                {/* Content overlay */}
                <div style={{ position: cardData.bannerImageUrl ? 'absolute' : 'relative' as const, bottom: cardData.bannerImageUrl ? 0 : undefined, left: 0, right: 0, padding: cardData.bannerImageUrl ? '24px 28px 56px' : '76px 28px 56px' }}>
                  {/* Logo */}
                  ...
                  {cardData.profilePhotoUrl && (
                    <div style={{ position: 'absolute', right: 28, bottom: -40, zIndex: 3 }}>
          ...
          {templateLayout === 'mobile-business' && (
            <div style={{ width: '100%' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', background: '#ffffff', borderBottom: '1px solid #f0f0f0' }}>
                ...
                {!endorsementTotalsUnavailable && cardData.endorsementCount > 0 && (
          ...
          {isNonClassicCivic && !isFlagCivic && (
            <CivicCardSection
              ...
              highlightColor={activeColorScheme?.accent || '#dfc274'}
              proposals={[]}
              questions={[]}
              commitments={[]}
              recommendations={[]}
          ...
          {templateLayout === 'church' && (
            <div style={{ width: '100%', textAlign: 'center' as const }}>
              ...
              <div style={{
                width: '100%', height: cardData.bannerImageUrl ? 180 : 88, position: 'relative',
                ...
                {/* Cross watermark */}
                <div style={{
                  position: 'absolute', top: cardData.bannerImageUrl ? '50%' : 26, left: '50%', transform: 'translate(-50%, -50%)',
                  fontSize: cardData.bannerImageUrl ? 80 : 32, opacity: 0.12, color: '#ffffff', fontWeight: 200,
                }}>✝</div>
                {/* Accent line */}
                <div style={{
              ...
              <div style={{ padding: '16px 24px 22px' }}>
                ...
                {cardData.title && <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.86)', margin: '6px 0 0', fontWeight: 500 }}>{cardData.title}</p>}
                {cardData.bio && <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.84)', margin: '10px 0 0', lineHeight: 1.6, fontStyle: 'italic' }}>{cardData.bio}</p>}
              </div>
            </div>
          )}
          ...
          <div style={{
            ...
            ...(templateLayout === 'pro-card' ? { padding: '14px 28px 4px', marginBottom: 8, background: activeColorScheme?.cardBg || '#ffffff', color: proBodyText } : {}),
            ...(templateLayout === 'cover-card' ? { padding: '12px 28px 4px', marginBottom: 8, background: '#ffffff', color: '#1a1a2e', alignItems: 'flex-start' } : {}),
            ...
            {templateLayout !== 'full-width' && templateLayout !== 'premium-static' && templateLayout !== 'pro-card' && templateLayout !== 'cover-card' && templateLayout !== 'biz-traditional' && templateLayout !== 'biz-modern' && templateLayout !== 'biz-minimalist' && templateLayout !== 'mobile-business' && !isCleanCivic && !isRallyCivic && !isBoldCivic && (() => {
                    ...
                    {cardData.profilePhotoUrl ? (
                      <img 
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            ...
                            placeholder.style.cssText = `width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: ${sizeConfig.initials}px; font-weight: 700; color: ${bgIsActuallyLight ? '#475569' : '#f8fafc'}; background: linear-gradient(135deg, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0.05) 100%); border-radius: ${sizeConfig.borderRadius - 4}px;`;
                      ...
                      <div style={{
                        ...
                        <span style={{
                          ...
                          color: bgIsActuallyLight ? '#475569' : '#f8fafc',
                          textShadow: 'none',
            ...
            {templateLayout === 'pro-realtor' && (
              <p style={{ fontSize: '12px', fontWeight: '600', letterSpacing: '2px', textTransform: 'uppercase' as const, color: readableOnSolid(templateStyles.accentColor, activeColorScheme?.background || '#ffffff', 4.5), margin: '8px 0 4px 0', textAlign: 'center' as const, opacity: 0.7 }}>HI, I&apos;M</p>
            )}
            ...
            {cardData.title && templateLayout !== 'pro-card' && templateLayout !== 'biz-traditional' && templateLayout !== 'biz-modern' && templateLayout !== 'biz-minimalist' && templateLayout !== 'mobile-business' && !isCleanCivic && <p style={{
              ...
              ...fontStyleOverrides,
              ...(templateLayout === 'pro-creative' ? { textAlign: 'left' as const, width: '100%' } : {}),
              ...(templateLayout === 'blogger' ? { fontSize: '12px', fontWeight: '600', letterSpacing: '1.5px', textTransform: 'uppercase' as const, opacity: 1, color: secondaryText } : {}),
            ...
            {cardData.company && templateLayout !== 'pro-card' && templateLayout !== 'biz-traditional' && templateLayout !== 'biz-modern' && templateLayout !== 'biz-minimalist' && templateLayout !== 'mobile-business' && <p style={{
              ...
              ...fontStyleOverrides,
              ...(templateLayout === 'pro-creative' ? { textAlign: 'left' as const, width: '100%' } : {}),
              ...(templateLayout === 'blogger' ? { fontSize: '12px', letterSpacing: '1px', color: readableOnSolid(templateStyles.accentColor, activeColorScheme?.cardBg || '#ffffff', 4.5), opacity: 1, fontWeight: '600' } : {}),
              ...(templateLayout === 'business-card' ? { color: '#64748b', opacity: 1 } : {}),
              ...(templateLayout === 'pro-realtor' ? { fontSize: '13px', fontWeight: '500' } : {}),
              ...(templateLayout === 'cover-card' ? { textAlign: 'left' as const, width: '100%', color: '#64748b', opacity: 1, fontSize: '14px' } : {}),
              ...(isFlagCivic ? { color: '#ffffff', opacity: 1, textShadow: '0 1px 6px rgba(0,0,0,0.45)' } : {}),
            }}>{cardData.company}</p>}
            ...
            {cardData.bio && templateLayout !== 'biz-traditional' && templateLayout !== 'biz-modern' && templateLayout !== 'biz-minimalist' && templateLayout !== 'mobile-business' && (
              <p style={{
                ...
                ...(templateLayout === 'pro-card' ? { textAlign: 'left' as const, width: '100%', padding: '0', color: proBodyText, fontSize: '14px', lineHeight: '1.6' } : {}),
                ...(templateLayout === 'business-card' ? { color: '#555', fontSize: '14px' } : {}),
                ...(templateLayout === 'blogger' ? { color: secondaryText, fontSize: '14px', lineHeight: '1.6' } : {}),
            ...
            {/* Corporate industry is explicit data; prose biography is never inferred into services. */}
            {templateLayout === 'pro-corporate' && cardData.professionalCategory && (
              <div style={{ width: '100%', marginTop: 16, zIndex: 1, position: 'relative' as const }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 6 }}>
                  <span style={{ padding: '6px 14px', borderRadius: 20, background: `${templateStyles.accentColor}15`, border: `1px solid ${templateStyles.accentColor}25`, fontSize: 13, fontWeight: 600, color: templateStyles.textColor }}>{cardData.professionalCategory}</span>
                </div>
              </div>
            )}
            ...
            <div style={{ width: '100%', padding: '0 24px 20px', background: '#FFFFFF' }}>
              ...
              {cardData.phone && (
                <a href={`tel:${cardData.phone}`} style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#1a1a2e' }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: `${templateStyles.accentColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: readableOnSolid(templateStyles.accentColor, '#ffffff', 3) }}>
                    <PhoneIcon />
                  </div>
                  <div style={{ flex: 1 }}>
          ...
          {templateLayout === 'pro-card' && cardData.showContactInfo && (
            <div style={{ width: '100%', padding: '0 28px 16px', background: activeColorScheme?.cardBg || '#ffffff' }}>
              {cardData.phone && (
                <a href={`tel:${cardData.phone}`} style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: `1px solid ${proBodyBorder}`, textDecoration: 'none', color: proBodyText }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${templateStyles.accentColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: proBodyAccent, flexShrink: 0 }}>
                    <PhoneIcon />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: proBodyText }}>{cardData.phone}</div>
                    <div style={{ fontSize: 11, color: proBodyMuted, fontWeight: 500 }}>Phone</div>
                  </div>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                </a>
              )}
              {cardData.phone && (
                <a href={`sms:${cardData.phone}`} style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: `1px solid ${proBodyBorder}`, textDecoration: 'none', color: proBodyText }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${templateStyles.accentColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: proBodyAccent, flexShrink: 0 }}>
                    <MessageIcon />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: proBodyText }}>Send a Text</div>
                    <div style={{ fontSize: 11, color: proBodyMuted, fontWeight: 500 }}>Message</div>
                  </div>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                </a>
              )}
              {cardData.email && (
                <a href={`mailto:${cardData.email}`} style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: `1px solid ${proBodyBorder}`, textDecoration: 'none', color: proBodyText }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${templateStyles.accentColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: proBodyAccent, flexShrink: 0 }}>
                    <EmailIcon />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: proBodyText, wordBreak: 'break-all' as const }}>{cardData.email}</div>
                    <div style={{ fontSize: 11, color: proBodyMuted, fontWeight: 500 }}>Email</div>
                  </div>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                </a>
              )}
              {cardData.website && (
                <a href={cardData.website.startsWith('http') ? cardData.website : `https://${cardData.website}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: `1px solid ${proBodyBorder}`, textDecoration: 'none', color: proBodyText }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${templateStyles.accentColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: proBodyAccent, flexShrink: 0 }}>
                    <GlobeIcon />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: proBodyText }}>{cardData.websiteLabel || 'Website'}</div>
                    <div style={{ fontSize: 11, color: proBodyMuted, fontWeight: 500 }}>{cardData.website.replace(/^https?:\/\//, '')}</div>
                  </div>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                </a>
              )}
              {fullAddress && (
                <a href={`https://maps.google.com/?q=${encodeURIComponent(fullAddress)}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: `1px solid ${proBodyBorder}`, textDecoration: 'none', color: proBodyText }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${templateStyles.accentColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: proBodyAccent, flexShrink: 0 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: proBodyText }}>{fullAddress}</div>
                    <div style={{ fontSize: 11, color: proBodyMuted, fontWeight: 500 }}>Location</div>
                  </div>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                </a>
              )}
          ...
          {templateLayout === 'cover-card' && cardData.showContactInfo && (() => {
            const iconAccent = cardData.buttonColor || templateStyles.accentColor;
            const iconBg = /^#[0-9a-f]{6}$/i.test(iconAccent) ? `${iconAccent}14` : '#f1f5f9';
            const iconForeground = readableOnSolid(cardData.iconColor || iconAccent, '#ffffff', 3);
            return (
            <div style={{ width: '100%', padding: '0 28px 16px', background: '#ffffff' }}>
              {cardData.email && (
                <a href={`mailto:${cardData.email}`} style={{ display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#333' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: iconBg, color: iconForeground, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, flexShrink: 0 }}>
                    <EmailIcon />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {templateLayout === 'biz-traditional' && cardData.showContactInfo && (
            <div style={{ width: '100%', padding: '0 28px 16px', background: '#ffffff' }}>
              {cardData.phone && (
                <a href={`tel:${cardData.phone}`} style={{ display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#333' }}>
                  <div style={{ width: 38, height: 38, borderRadius: '50%', background: `${templateStyles.accentColor}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, flexShrink: 0 }}>
                    <PhoneIcon />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 500, color: '#333' }}>{cardData.phone}</div>
              ...
              {cardData.socialInstagram && (
                <a href={`https://instagram.com/${cardData.socialInstagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#333' }}>
                  <div style={{ width: 38, height: 38, borderRadius: '50%', background: `${socialIconColor}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, flexShrink: 0 }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill={socialIconColor}><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {templateLayout === 'biz-modern' && cardData.showContactInfo && (
            <div style={{ width: '100%', padding: '0 28px 16px', background: '#ffffff' }}>
              {cardData.phone && (
                <a href={`tel:${cardData.phone}`} style={{ display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#333' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${templateStyles.accentColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: readableOnSolid(templateStyles.accentColor, '#ffffff', 3), flexShrink: 0 }}>
                    <PhoneIcon />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {cardData.socialInstagram && (
                <a href={`https://instagram.com/${cardData.socialInstagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#333' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${socialIconColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: socialIconColor, flexShrink: 0 }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill={socialIconColor}><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {cardData.socialLinkedin && (
                <a href={`https://linkedin.com/in/${cardData.socialLinkedin}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#333' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${socialIconColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: socialIconColor, flexShrink: 0 }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill={socialIconColor}><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {cardData.socialTiktok && (
                <a href={`https://tiktok.com/${cardData.socialTiktok.replace('@', '')}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: '#333' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: `${socialIconColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 14, color: socialIconColor, flexShrink: 0 }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill={socialIconColor}><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 0010.86 4.46V13a8.28 8.28 0 005.58 2.16V11.7a4.83 4.83 0 01-3.77-1.24V6.69h3.77z"/></svg>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                {cardData.phone && (
                  <a href={`tel:${cardData.phone}`} style={{
                    ...
                    background: templateStyles.accentColor, color: readableOnSolid('#ffffff', templateStyles.accentColor, 4.5),
          ...
          {templateLayout !== 'business-card' && templateLayout !== 'pro-card' && templateLayout !== 'cover-card' && templateLayout !== 'biz-traditional' && templateLayout !== 'biz-modern' && templateLayout !== 'biz-minimalist' && templateLayout !== 'mobile-business' && cardData.showContactInfo && Boolean(cardData.phone || cardData.email || cardData.website) && <div style={{
            ...
            {cardData.phone && (
              <a href={`tel:${cardData.phone}`} className="action-btn" style={{
                ...
                color: readableOnSolid(templateStyles.textColor, buttonStyleOverrides.background || socialButtonBackground, 4.5),
                ...buttonStyleOverrides,
              }}>
                <div style={styles.actionIconWrapper}>
                  <PhoneIcon />
                </div>
                <span style={{...styles.actionText, color: readableOnSolid(templateStyles.textColor, buttonStyleOverrides.background || socialButtonBackground, 4.5)}}>Call</span>
              </a>
            )}
            {cardData.phone && (
              <a href={`sms:${cardData.phone}`} className="action-btn" style={{
                ...
                <span style={{...styles.actionText, color: readableOnSolid(templateStyles.textColor, buttonStyleOverrides.background || socialButtonBackground, 4.5)}}>Text</span>
              </a>
            )}
            {cardData.email && (
              <a href={`mailto:${cardData.email}`} className="action-btn" style={{
                ...
                <span style={{...styles.actionText, color: readableOnSolid(templateStyles.textColor, buttonStyleOverrides.background || socialButtonBackground, 4.5)}}>Email</span>
              </a>
            )}
            {cardData.website && (
              <a 
              ...
              >
                ...
                <span style={{...styles.actionText, color: readableOnSolid(templateStyles.textColor, buttonStyleOverrides.background || socialButtonBackground, 4.5)}}>{cardData.websiteLabel || 'Website'}</span>
              </a>
            )}
          </div>}
              ...
              {cardData.featuredSocials.map((item, index) => {
                ...
                return (
                  <a
                    ...
                    title={config.label}
                  >
                    <svg viewBox="0 0 24 24" width="22" height="22" fill={readableOnSolid('#ffffff', cardData.socialIconColor || config.bgColor)}>
            ...
            <div style={styles.socialLinks}>
              {showInstagram && (
                <a 
                  ...
                  title="Instagram"
                >
                  <svg viewBox="0 0 24 24" width="20" height="20" fill={socialButtonIconColor}>
              ...
              {cardData.recentEndorsements.slice(0, 3).map((endorsement, idx) => (
                <div key={idx} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: '#999', marginLeft: 'auto' }}>{endorsement.createdAt ? new Date(endorsement.createdAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : ''}</span>
                  </div>
                  <p style={{ fontSize: 13, color: '#555', lineHeight: 1.5, margin: 0, fontStyle: 'italic' }}>"{endorsement.note}"</p>
                  {!previewOnly && endorsement.id && <ContentSafetyActions kind="ecard_endorsement" contentId={endorsement.id}/>}
                </div>
              ))}
              {/* Endorse CTA */}
              <button
          ...
          {!previewOnly && <div style={{width:'100%',maxWidth:400,margin:'12px auto'}}><ContentSafetyActions kind="ecard" contentId={cardData.id}/></div>}
            ...
            <div style={styles.formBlock}>
              ...
              {formSubmitted ? (
                ...
                <>
                  ...
                  ) : !previewOnly && cardData.formBlock.formType === 'gohighlevel' && cardData.formBlock.ghlEmbedCode && !cardData.formBlock.ghlWebhookUrl ? (
                    /* GHL Embed Form */
                    <>
                      {showFormEmbed && !previewOnly ? (
                        <div style={styles.formEmbedContainer}>
              ...
              {cardData.links.map((link, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{...styles.linkButtonText, color: (templateLayout === 'business-card' || templateLayout === 'pro-card' || templateLayout === 'cover-card') ? readableOnSolid('#1a1a2e', cardData.buttonColor || '#f8f9fa', 4.5) : templateLayout === 'blogger' ? secondaryText : readableOnSolid(templateStyles.textColor, socialButtonBackground, 4.5)}}>{link.title}</span>
      ...
      {showEndorsementPopup && cardData && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 13, color: '#888', marginTop: -14, marginBottom: 16 }}>{endorsementTotalsUnavailable ? t('endorsement.totalsUnavailable','Your endorsement was saved. Updated totals could not be loaded.') : t('endorsement.totalCount', { count: cardData.endorsementCount ?? cardData.tapCount ?? 0 })}</div>

            {/* Top Endorsement Tags — shown as rows with ×count */}
            {endorsementTotalsUnavailable ? null : cardData.topEndorsementTags && cardData.topEndorsementTags.length > 0 ? (
