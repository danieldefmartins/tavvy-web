...
export default function SignalCard({place,onClick}:{place:SignalCardPlace;onClick?:()=>void}) {
  ...
  return <article className="card" aria-label={place.name}>
    ...
    <div className={'photos '+(photos.length>1?'multiple':'')} aria-label={copy('Photos')}>
      {photos.map((src,i)=><button className="photo" onClick={onClick} type="button" key={src} aria-label={place.name+' · '+copy(isCategory?'Category illustration':'Photos')}>
        <img src={src} loading="lazy" alt={isCategory?copy('Category illustration'):place.name+' · '+(i+1)} onError={()=>{if(!isCategory)setFailedPhotos(previous=>[...previous,src]);}}/>
        {isCategory&&<span className="illustration">{copy('Category illustration')}</span>}
      </button>)}
    </div>
    ...
    <style jsx>{`
.card{width:100%;overflow:hidden;background:${theme.cardBackground};color:${theme.text};border:1px solid ${theme.border};border-radius:18px;margin-bottom:14px;box-shadow:0 3px 14px ${isDark?"rgba(0,0,0,.18)":"rgba(23,1,58,.06)"}}
...
.identity{display:block;text-align:left;padding:16px 16px 12px;width:100%;border:0;background:none}
h3{font-size:19px;line-height:1.3;margin:0 0 6px;font-weight:750;overflow-wrap:anywhere}
...
.photos{display:flex;gap:8px;overflow-x:auto;scroll-snap-type:x proximity;padding:0 16px;scrollbar-width:none}
.photos::-webkit-scrollbar{display:none}.photo{position:relative;flex:0 0 100%;height:154px;padding:0;border:0;border-radius:12px;overflow:hidden;background:${theme.surface};scroll-snap-align:start}
.multiple .photo{flex-basis:65%}.photo img{display:block;width:100%;height:100%;object-fit:cover}
